...
public class SpiceResourceTreeFolder implements ResourceTreeFolder {
    ...
    public Optional<Resource> getResource(String... path) {
        Optional<ResourceTreeFolder> folder = getFolder(Arrays.copyOf(path, path.length - 1));
        return folder.isPresent() ? folder.get().getResource(path[path.length - 1]) : Optional.empty();
    }
