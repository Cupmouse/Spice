...
public final class SwingWindowDispatcher implements WindowDispatcher<SwingWindowSystem, SwingWindow> {
    ...
    private Map<Class<? extends Window>, Pair<Supplier<? extends SwingWindow>, WindowStateImmutable>> windowSuppliers = new HashMap<>();
    ...
    @Override
    public <K extends Window, W extends SwingWindow> void registerWindowSupplier(Class<K> clazz, Supplier<W> windowSupplier, WindowStateImmutable defaultState) throws AlreadyRegisteredException {
        Validate.nullNot(clazz, windowSupplier, defaultState);
        this.windowSuppliers.put(clazz, Pair.of(windowSupplier, defaultState));
    }
